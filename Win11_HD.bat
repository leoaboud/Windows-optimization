@echo off
:: ============================================================
::  DEV MASTER BY ABOUD - HDD EDITION (PROGRESS BAR)
:: ============================================================
...
title DEV MASTER BY ABOUD - OTIMIZACAO HDD
color 0A

echo =========================================================
echo               DEV MASTER (WIN 11 - HD)
echo    ESTA VERSAO INCLUI DESFRAGMENTACAO FISICA DO DISCO.
echo    O PC SERA DESLIGADO AUTOMATICAMENTE AO FINALIZAR.
echo =========================================================
echo.

:: Variável para a barra de progresso via PowerShell
set "PS_PROGRESS=powershell -NoProfile -ExecutionPolicy Bypass -Command "Write-Progress -Activity 'DEV MASTER (WIN 11 - HD)' -Status"

:: [1/13] DNS
%PS_PROGRESS% 'Limpando DNS (1/13)' -PercentComplete 8"
ipconfig /flushdns >nul 2>&1

:: [2/13] Rede e Energia (Hibernação ON para HDDs)
...
:: [7/13] Temporários
%PS_PROGRESS% 'Limpando Arquivos Temporarios (7/13)' -PercentComplete 54"
del /s /f /q "%TEMP%\*.*" >nul 2>&1
del /s /f /q "C:\Windows\Temp\*.*" >nul 2>&1

:: [8/13] Prefetch (Essencial para HDD)
%PS_PROGRESS% 'Limpando Prefetch (8/13)' -PercentComplete 61"
del /s /f /q "C:\Windows\Prefetch\*.*" >nul 2>&1

:: [9/13] Winget
%PS_PROGRESS% 'Atualizando Apps via Winget (9/13)' -PercentComplete 69"
winget upgrade --all --include-unknown --accept-package-agreements --accept-source-agreements >nul 2>&1

:: [10/13] SFC
%PS_PROGRESS% 'Validando Arquivos de Sistema (10/13)' -PercentComplete 77"
sfc /scannow >nul 2>&1

:: [11/13] WinSxS
%PS_PROGRESS% 'Otimizando Nucleo (11/13)' -PercentComplete 85"
:: Note: Removido /ResetBase para evitar travamentos em HDDs lentos
Dism.exe /online /Cleanup-Image /StartComponentCleanup >nul 2>&1

:: [12/13] DISM Final
%PS_PROGRESS% 'Consolidando Limpeza Profunda (12/13)' -PercentComplete 92"
Dism.exe /online /Cleanup-Image /StartComponentCleanup /NoRestart >nul 2>&1

:: [13/13] DESFRAGMENTAÇÃO (O passo mais importante para HDD)
%PS_PROGRESS% 'DESFRAGMENTANDO DISCO FISICO (13/13)' -PercentComplete 100"
:: Otimização de HDD (Consolidação de arquivos)
...
shutdown /s /t 10 /c "Manutencao DEV MASTER (HDD) finalizada. Reiniciando para aplicar mudancas fisicas no disco."
timeout /t 10